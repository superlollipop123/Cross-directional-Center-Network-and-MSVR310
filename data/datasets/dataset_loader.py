...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        img_path, pid, camid, sceneid = self.dataset[index]
        #pdb.set_trace()
        img1 = read_image(img_path[0])
        img2 = read_image(img_path[1])
        img3 = read_image(img_path[2])
        #print(img1)
        ...
        #print(img1)
        #pdb.set_trace()
        return (img1, img2, img3), pid, camid, sceneid, '/'.join(img_path[0].split('/')[-2:])

class ImageDataset_2(Dataset):
    """ For RNT100 and RN300"""
