...
from .circle_loss import fastreid_circle_loss
...
def make_loss_with_center(cfg, num_classes):    # modified by gu
    ...
    if cfg.MODEL.METRIC_LOSS_TYPE == 'center_only': # added by zxp, center loss
        center_criterion = CenterLoss(num_classes=cfg.SOLVER.CENTER_NUM, feat_dim=cfg.SOLVER.CENTER_F_DIM, use_gpu=True) 
    ...
    def loss_func(score, feat, target):
        if cfg.MODEL.METRIC_LOSS_TYPE == 'center_only': # zxp
            return cfg.SOLVER.CENTER_LOSS_WEIGHT * center_criterion(feat, target)   
