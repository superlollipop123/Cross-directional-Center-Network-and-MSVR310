...
# from modeling.baseline_zxp import MainNet as Test_Model
from v.save_feat.used_model import MainNet as Test_Model
...
def main():
    ...
    output_file = os.path.join('v', 'save_feat', '{}.pkl'.format(__MODEL_NAME))
...
def save_feat(cfg, model, data_loader, save_dict):
    ...
    #         save_dict[k] = np.asarray(torch.cat(save_dict[k], dim=0))
    # # output_file = os.path.join('v', 'tSNE', 'bs_hc_mmic_sc_test.pkl')
    # output_file = os.path.join('v', 'tSNE', '{}.pkl'.format(__MODEL_NAME))
    # with open(output_file, 'wb') as f:
    #     pickle.dump(save_dict, f)
    # print('feats saved')
...
def evaluator_savefeat(model, device=None, data_saver=None):
    ...
    def _inference(engine, batch):
        ...
        with torch.no_grad():
            ...
            ori_x = model([data1, data2, data3])
            feat = None
            B = data1.shape[0]
            ori_mean = [x.mean(dim=[1,2,3]) for x in ori_x]
            ori_std = [x.view(B, -1).std(dim=1) for x in ori_x]
            ori_mean = torch.stack(ori_mean, dim=1)
            ori_std = torch.stack(ori_std, dim=1)

            # normed_mean = [x.mean(dim=[1,2,3]) for x in normed_x]
            # normed_std = [x.view(B, -1).std(dim=1) for x in normed_x]
            # normed_mean = torch.stack(normed_mean, dim=1)
            # normed_std = torch.stack(normed_std, dim=1)

            # data_saver['f_before_norm'].append(torch.cat(normed_x, dim=1).detach().cpu())
            # data_saver['f_after_norm'].append(torch.cat(ori_x, dim=1).detach().cpu())
            data_saver['f_ori_mean'].append(ori_mean.detach().cpu())
            data_saver['f_ori_std'].append(ori_mean.detach().cpu())
            # data_saver['f_normed_mean'].append(normed_mean.detach().cpu())
            # data_saver['f_normed_std'].append(normed_std.detach().cpu())

            # feat = torch.cat(g_feats, dim=1)
            # data_saver['gf_feat'].append(feat.detach().cpu())
            # data_saver['bn_f_feat'].append(torch.cat(bn_f_list, dim=1).detach().cpu())
