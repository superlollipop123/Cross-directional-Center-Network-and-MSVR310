...
class Baseline(nn.Module):
    ...
    def forward(self, x):
        ...
        # pdb.set_trace()
        # normed_x, ori_x = self.modalnorm(x)
...
class MainNet(nn.Module):
    ...
    def __init__(self, num_classes, last_stride, model_path, neck, neck_feat, model_name, pretrain_choice, branches=2):
    ...
    def forward(self, inputs):
        ...
        normed_x, ori_x = [], []
        for i in range(self.branches):
            # x_n, x_o = self.__getattr__('branch_'+str(i)).forward(bra_mid_feats[i]) # stage 2 for branches
            x_o = self.__getattr__('branch_'+str(i)).forward(bra_mid_feats[i]) # stage 2 for branches
            # normed_x.append(x_n)
            ori_x.append(x_o)

        # stage 2 for fused branch
        # fused_gf, fused_bn_f, fused_pred, fused_w = self.fuse_branch(ds_fused_feat)
        # gf_list.append(fused_gf)
        # bn_f_list.append(fused_bn_f)
        # pred_list.append(fused_pred)
        # weight_list.append(fused_w)
